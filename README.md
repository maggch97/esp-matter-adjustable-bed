...
```bash
# 设置目标芯片（以 ESP32-S3 为例），会根据 sdkconfig.defaults 自动生成 sdkconfig
./idf.sh set-target esp32s3

# 编译
./idf.sh build

# 擦除 flash（首次烧录或需要清除配网信息时）
./idf.sh erase-flash
...
> **注意**：`set-target` 会清空 `build/` 目录并从 `sdkconfig.defaults` 重新生成 `sdkconfig`。如果修改了 `sdkconfig.defaults`，需要先删除 `sdkconfig` 再重新 `set-target`：
> ```bash
> rm sdkconfig
> ./idf.sh set-target esp32s3
> ```
...
# 2. 克隆本项目
cd ~/workspace
git clone https://github.com/maggch97/esp-matter-adjustable-bed.git
cd esp-matter-adjustable-bed

# 3. 设置目标并编译（component manager 会自动下载 esp_matter 等依赖）
./idf.sh set-target esp32s3
./idf.sh build
